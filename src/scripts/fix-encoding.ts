...
const dedupedData = vocabularyData.filter((item: any) => {
  if (seen.has(item.word)) {
    console.log(`删除重复词汇: ${item.word} (${item.meaning})`);
    return false;
  }
  seen.add(item.word);
  return true;
...
console.log(`去重完成！`);
console.log(`原始词汇数量: ${vocabularyData.length}`);
console.log(`去重后数量: ${dedupedData.length}`);
console.log(`删除了 ${vocabularyData.length - dedupedData.length} 个重复词汇`); 